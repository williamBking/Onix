...
(function () {
  ...
  function paintDashboardView(data) {
    ...
    // Inject a live portfolio summary + top-clients table once, directly on the
    // dashboard, so real data is visible without navigating to the Clients tab.
    if (!v.querySelector('#onix-live-panel')) {
      const pageHd = v.querySelector('.page-hd');
      const panel = document.createElement('div');
      panel.id = 'onix-live-panel';

      // Build top-clients rows (sorted by total active loan balance, top 10)
      const loanBalMap = {};
      const loanCntMap = {};
      activeLoans.forEach(l => {
        loanBalMap[l.user_id] = (loanBalMap[l.user_id] || 0) + Number(l.balance || 0);
        loanCntMap[l.user_id] = (loanCntMap[l.user_id] || 0) + 1;
      });
      const activeClientList = clients.filter(c => c.role === 'client' && c.status === 'active');
      const topClients = activeClientList
        .slice()
        .sort((a, b) => (loanBalMap[b.id] || 0) - (loanBalMap[a.id] || 0))
        .slice(0, 10);
      const clientRows = topClients.map(c =>
        '<tr>' +
          '<td style="padding:8px 10px;border-bottom:1px solid #f4f4f4;font-size:.82rem">' + esc(c.full_name || c.email || '—') + '</td>' +
          '<td style="padding:8px 10px;border-bottom:1px solid #f4f4f4;font-size:.82rem;color:#C0392B;font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:1rem">' + (loanBalMap[c.id] ? fmt.money(loanBalMap[c.id]) : '—') + '</td>' +
          '<td style="padding:8px 10px;border-bottom:1px solid #f4f4f4;font-size:.82rem;color:#888">' + (loanCntMap[c.id] || '—') + ' loan' + (loanCntMap[c.id] === 1 ? '' : 's') + '</td>' +
        '</tr>'
      ).join('');

      panel.innerHTML =
        '<div style="margin-bottom:24px;font-family:\'DM Sans\',sans-serif">' +
          // Portfolio summary bar
          '<div style="background:#1A1A1A;color:#fff;padding:14px 20px;margin-bottom:16px;display:flex;gap:32px;flex-wrap:wrap;align-items:center">' +
            '<div><div style="font-size:.6rem;letter-spacing:.15em;text-transform:uppercase;color:rgba(255,255,255,.5);margin-bottom:4px">Active Loan Portfolio</div>' +
              '<div style="font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:1.8rem;color:#C0392B">' + fmt.money(loanPortfolio) + '</div></div>' +
            '<div><div style="font-size:.6rem;letter-spacing:.15em;text-transform:uppercase;color:rgba(255,255,255,.5);margin-bottom:4px">Active Loans</div>' +
              '<div style="font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:1.8rem">' + activeLoans.length + '</div></div>' +
            '<div><div style="font-size:.6rem;letter-spacing:.15em;text-transform:uppercase;color:rgba(255,255,255,.5);margin-bottom:4px">Active Clients</div>' +
              '<div style="font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:1.8rem">' + activeClientList.length + '</div></div>' +
          '</div>' +
          // Top clients table
          '<div style="background:#fff;border:1px solid #E8E8E8;border-top:3px solid #C0392B">' +
            '<div style="padding:12px 16px;border-bottom:1px solid #E8E8E8;display:flex;justify-content:space-between;align-items:center">' +
              '<span style="font-size:.65rem;letter-spacing:.14em;text-transform:uppercase;color:#1A1A1A;font-weight:700">Top Clients by Loan Balance</span>' +
              '<span style="font-size:.7rem;color:#888">Click <strong>Clients</strong> in the sidebar for full list · <strong>Calendar</strong> for ' + activeLoans.length + ' loan closings</span>' +
            '</div>' +
            '<table style="width:100%;border-collapse:collapse">' +
              '<thead><tr>' +
                '<th style="background:#f4f4f4;padding:8px 10px;text-align:left;font-size:.62rem;letter-spacing:.1em;text-transform:uppercase;color:#888;font-weight:700">Client Name</th>' +
                '<th style="background:#f4f4f4;padding:8px 10px;text-align:left;font-size:.62rem;letter-spacing:.1em;text-transform:uppercase;color:#888;font-weight:700">Total Loan Balance</th>' +
                '<th style="background:#f4f4f4;padding:8px 10px;text-align:left;font-size:.62rem;letter-spacing:.1em;text-transform:uppercase;color:#888;font-weight:700">Loans</th>' +
              '</tr></thead>' +
      ...
      if (pageHd && pageHd.nextSibling) {
        pageHd.parentNode.insertBefore(panel, pageHd.nextSibling);
      } else if (pageHd) {
        pageHd.parentNode.appendChild(panel);
      } else {
